...
This is a duplicate of scripts/shelf_changes.py (the API layer never
imports from scripts/, per web/api's self-contained-serverless-function
convention — see web/api/db.py duplicating scripts/db_connection.py).
Everything above this point must stay byte-identical to that file. Below,
conquest_breadth is a deliberate, one-way divergence added for the API
layer only — it does not exist in scripts/shelf_changes.py and should not
be backported there.
"""
import re
