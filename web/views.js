...
export function renderLeaderboard() {
  ...
  document.getElementById('leaderboard').innerHTML = `
    <table class="lb"><thead><tr>
      ...
      <th>Archetype</th><th>Action</th><th>Brands</th></tr></thead><tbody>
    ${rows.map((l, i) => `<tr>
      <td class="mono">${i + 1}</td><td>${l.AREA}</td><td>${l.ADDRESS}</td>
      ...
      <td>${gtmDot(l.gtm_action)}</td><td class="mono">${l.n_brands_confirmed}/3</td></tr>`).join('')}
    </tbody></table>`;
}
...
export function renderGems() {
  const table = (title, rows, extraHead, extraCell) => `
    ...
    ${rows.slice(0, 25).map((l) => `<tr><td>${l.AREA}</td><td>${l.ADDRESS}</td>
  ...
  const t = (k) => (v) => v === true || v === 'true' || v === 'True';
